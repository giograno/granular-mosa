...
def getEvoSuiteCall(seed, configId, config, project, clazz, id, strategy, coreIndex):
    ...
    result += ' -Doutput_variables=TARGET_CLASS,criterion,configuration_id,'\
        'Total_Branches,Lines,Covered_Goals,Generations,Statements_Executed,Fitness_Evaluations,Tests_Executed,Generations,Total_Time,'\
        'Size,Length,Result_Length,Result_Size,'\
        'Random_Seed,'\
        'BranchCoverage,LineCoverage,WeakMutationScore,ExceptionCoverage,InputCoverage,OutputCoverage,MethodCoverage,MutationScore,'\
        'NoEagerTests,Result_NoEagerTests'
    if strategy == '-boosted':
      # statistics for boosted approach
        result += ",FirstStep_Goals," \
            "SecondStep_Goals," \
        ...
        result += " -Dstrategy=BOOSTED"
    result += " -Dgroup_id="+project
    result += " -Dalgorithm="+ALGORITHM
    result += " -Dsearch_budget="+str(BUDGET)
    result += " "+config+" "+FIXED
    result += " -Dreport_dir="+reportfile
    result += " -Dtest_dir=" + TESTDIR + "/s" + \
        str(seed) + "/" + project + "/evosuite-tests"
...
def createJobs(minSeed, maxSeed, configId, config, startNum, strategy="-generateSuite"):
    ...
    for seed in range(minSeed, maxSeed):
        ...
        for entry in CLASSES:
            ...
            script.write(getEvoSuiteCall(seed, configId, config,
                         entry[0], entry[1], JOB_ID, strategy, coreIndex))
...
ENTRIES_PER_JOB = math.ceil(
    (N_CONF * (NUM_CLASSES * (MAXSEED - MINSEED)) / float(MAX_JOBS)))
