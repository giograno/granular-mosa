...
def getEvoSuiteCall(seed, configId, config, project, clazz, id, strategy, coreIndex):
  ...
  result += ' -Doutput_variables=TARGET_CLASS,criterion,configuration_id,Total_Branches,Lines,Covered_Goals,Generations,Statements_Executed,Fitness_Evaluations,Tests_Executed,Generations,Total_Time,Size,Length,Result_Length,Result_Size,Random_Seed,BranchCoverage,LineCoverage,WeakMutationScore,ExceptionCoverage,InputCoverage,OutputCoverage,MethodCoverage,MutationScore,NoEagerTest,FirstStepSize,SecondStepSize'
  if strategy == '-boosted':
    result += ',FirstStepGoals,SecondStepGoals'
  result += " -Dgroup_id="+project
  result += " -Dalgorithm="+ALGORITHM
  if ALGORITHM == 'SMOSA':
    result += ' -Dmap_CUT_calls=true'
    result += ' -Dno_et='+str(NO_ET)
